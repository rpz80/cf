...
```
Note though, that timeout is set at the point of `cf::future::timeout` invocation creation ,i.e. all timeouts in the example above are scheduled almost at the same moment. Thus when calling `cf::future::timeout` second and subsequent times condsider adding up approximate duration of the previous calls when choosing timeout values.
